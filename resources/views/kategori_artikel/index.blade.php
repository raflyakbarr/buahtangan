                        ...
                        <ul class="list-group">
                            @foreach($categories as $category)
                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                    ...
                                    <div>
                                        <span class="badge bg-primary rounded-pill me-2">{{ $category->articles->count() }}</span>
                                        <button type="button" class="btn btn-sm btn-warning me-2" data-bs-toggle="modal" data-bs-target="#editModal{{ $category->id }}">
                                            Edit
                                        </button>
                                        <form action="{{ route('kategori_artikel.destroy', $category->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                ...
                                <div class="modal fade" id="editModal{{ $category->id }}" tabindex="-1" aria-labelledby="editModalLabel{{ $category->id }}" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <form action="{{ route('kategori_artikel.update', $category->id) }}" method="POST">
                                                @csrf
                                                @method('PUT')
